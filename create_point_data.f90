...
integer       :: daily_met      ! 1: daily met inputs 
...
integer           :: itime
integer           :: n_args
character(len=27) :: namelist_filename
character(len=27) :: filename
character(len=27) :: filename_met
character(len=49) :: target_folder  ! save all inputs required ...
...
namelist / location / latitude, longitude, vegetation_category, soil_category, &
  ...
  minimum_vegetation_pct, land_mask, nsoil, target_folder, filename_met, daily_met
...
namelist_filename = "namelist.met"
endif
WRITE(*,*) namelist_filename

open(30, file=TRIM(adjustl(namelist_filename)), form="FORMATTED")
read(30, location)
...
! target_folder = "HRLDAS_in"

! write(*,*) TRIM(adjustl(target_folder))

! Create the NetCDF file mimicing WRF input files.

  iret = nf90_create(TRIM(adjustl(target_folder))//"/"//"hrldas_setup_single_point.nc", NF90_CLOBBER, ncid)
...
open(30, file=TRIM(adjustl(filename_met)), form="FORMATTED")
...
! write(*,*) "Creating: "//filename
  ...
  iret = nf90_create(TRIM(adjustl(target_folder))//"/"//filename, NF90_CLOBBER, ncid)
